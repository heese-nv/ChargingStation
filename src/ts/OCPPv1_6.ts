...
class OCPPv1_6 {
    ...
    constructor(WriteToScreen: WriteToScreenDelegate,
        ...
        // this.websocket  = new WebSocket(wsUri ?? "ws://localhost:8180/steve/websocket/CentralSystemService/1234", "ocpp1.6"); // Steve Docker
        this.websocket = new WebSocket(wsUri ?? "ws://localhost:8080/steve/websocket/CentralSystemService/1234", "ocpp1.6"); // Steve Dev
        ...
        this.websocket.onmessage = (e) => {
            this.WriteToScreen("<span>RECEIVED: " + e.data + "</span>");
            ...
            if (Array.isArray(data)) {
                ...
                // message is a
                if (messageType === 2) {
                    this.sendStatusResponse(messageId, "Accepted");
                }
            }
        };
    ...
    public sendStatusResponse(messageId: string, status: string) {
        let data = JSON.stringify([3, messageId, {status: status}]); // 3 => call result
        this.sendRAWRequest(data)
    }
